...
public class UiState {
    ...
    public void setMetric(DistanceStrategy metric) {
        ...
        for (UiStateListener l : listeners) l.onMetricChanged(this.metric);
    }


    public void setPrimaryResults(List<Neighbor> results) {
        this.primaryResults = (results == null) ? List.of() : List.copyOf(results);
        for (UiStateListener l : listeners) l.onPrimaryResultsChanged(this.primaryResults);
    }

    public void setHighlightedKeys(Set<String> keys) {
        this.highlightedKeys = (keys == null) ? Set.of() : Set.copyOf(keys);
        for (UiStateListener l : listeners) l.onHighlightsChanged(this.highlightedKeys);
    }

    public void setStatus(String msg) {
        this.status = (msg == null) ? "" : msg;
        for (UiStateListener l : listeners) l.onStatusChanged(this.status);
    }

    public void setError(String msg) {
        this.error = (msg == null) ? "" : msg;
        for (UiStateListener l : listeners) l.onErrorChanged(this.error);
    }
}
